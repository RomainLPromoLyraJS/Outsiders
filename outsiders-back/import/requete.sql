...
SELECT t.id as trip_id, t.title as trip_title, t.description as trip_description, t.date, t.time, t.from, t.to, t.places, t.minimum, t.price, t.duration, s.id AS sport_id, s.title AS sport_title, s.description AS sport_description, JSON_AGG(JSON_build_object(\'id\', u.id, \'username\', u.username, \'email\', u.email, \'description\', u.description)) AS "creator", JSON_AGG(JSON_build_object(\'id\', m.id, \'title\', m.title, \'content\', m.content, \'date\', m.date, \'user_id\', m.user_id, \'username\', "user".username) ORDER BY m.id) AS "message" FROM trip AS t JOIN sport AS s ON t.sport_id=s.id JOIN "message" AS m on t.id=m.trip_id JOIN "user" AS u ON t.user_id=u.id JOIN "user" ON m.user_id="user"."id" WHERE t.id=$1 GROUP BY t.id, t.title, t.description, t.date, t.time, t.from, t.to, t.places, t.minimum, t.price, t.duration, s.id, s.title, s.description
...
select * from m2m_user_participate_trip m Join trip on m.trip_id=trip.id where m.user_id=$1
